...
function dacnis_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'dacnis_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function dacnis_theme() {
	return Theme::getInstance();
}

function dacnis_assets() {
	return dacnis_theme()->getAssetsManager();
}
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	dacnis_assets()
		...
		->registerStylesheet( 'dacnis-theme', '/theme/theme.css' );
	dacnis_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		dacnis_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'dacnis_print_color_scheme', 0 );
} else {
	dacnis_assets()
		->registerStyle( 'dacnis-theme', dacnis_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function dacnis_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'dacnis_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'dacnis_after_activation_redirect_url' );
function dacnis_after_activation_redirect_url( $url ) {
...
function dacnis_get_builder_plugin_slug() {
...
function dacnis_theme_plugins( $plugins ) {

	$plugin_slug = dacnis_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'dacnis' ) : __( 'Kubio', 'dacnis' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'dacnis_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === dacnis_get_builder_plugin_slug() ) {
				...
				<div class="kubio-admin-row get-started-imported kubio-notice notice notice-success">
					<div>
						<p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'dacnis' ),
									dacnis_theme()->getName()
								)
							);
							?>
						</p>
						<p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'dacnis' ),
									dacnis_theme()->getName()
								)
							);
							?>
...
function dacnis_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'dacnis_try_online_url', 10 );
...
add_action( 'wp_head', 'dacnis_render_header_style', 100 );
