...
(function ($) {
  var $notice_container = $(".dacnis-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.dacnis_admin) {
    return;
  }

  var _window$dacnis_admin = window.dacnis_admin,
      builderStatusData = _window$dacnis_admin.builderStatusData,
      getStartedData = _window$dacnis_admin.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.dacnis-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".dacnis-admin-big-notice").addClass("processing");
      $(".dacnis-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".dacnis-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.dacnis-admin-big-notice");
    });
  }
  ...
  function processBuilderInstalationStepts(_x) {
    return _processBuilderInstalationStepts.apply(this, arguments);
  }

  function _processBuilderInstalationStepts() {
    _processBuilderInstalationStepts = _asyncToGenerator( /*#__PURE__*/_babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default().mark(function _callee(callback) {
      var _ref,
          _ref$AI,
          AI,
          _ref$source,
  ...
  $notice_root = $notice_container.closest(".dacnis-admin-big-notice");
  $custom_close_button = $notice_root.find(".dacnis-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".dacnis-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.dacnis_admin.showOverlay = showOverlay;
})(jQuery);
})();
